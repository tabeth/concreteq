...
func (dl directoryLayer) List(rt fdb.ReadTransactor, path []string, options ListOptions) ([]string, error) {
	r, err := rt.ReadTransact(func(rtr fdb.ReadTransaction) (interface{}, error) {
		...
		if node.isInPartition(nil, true) {
			...
			return nc.List(rtr, node.getPartitionSubpath(), options)
		}

		return dl.subdirNames(rtr, node.subspace, options)
	})
	if err != nil {
		return nil, err
	}
...
func isRangeEmpty(rtr fdb.ReadTransaction, r fdb.Range) bool {
	kvs, err := rtr.GetRange(r, fdb.RangeOptions{Limit: 1}).GetSliceWithError()
	if err != nil {
		return false
	}
	return len(kvs) == 0
