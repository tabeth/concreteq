...
func TestDeleteMessageHandler_MalformedHandle(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("DeleteMessage", mock.Anything, "my-queue", "invalid-handle").Return(store.ErrInvalidReceiptHandle)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueUrl": "http://localhost/queues/my-queue", "ReceiptHandle": "invalid-handle"}`
...
func TestChangeMessageVisibilityHandler_MessageNotInflight(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("ChangeMessageVisibility", mock.Anything, "my-queue", "a-handle", 60).Return(store.ErrMessageNotInflight)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueUrl": "http://localhost/queues/my-queue", "ReceiptHandle": "a-handle", "VisibilityTimeout": 60}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
...
func TestGetQueueAttributesHandler_QueueNotFound(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("GetQueueAttributes", mock.Anything, "non-existent").Return(nil, store.ErrQueueDoesNotExist)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueUrl": "http://localhost/queues/non-existent", "AttributeNames": ["All"]}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
...
func TestListQueuesHandler_NoMatch(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("ListQueues", mock.Anything, 0, "", "no-match").Return(nil, "", nil)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueNamePrefix": "no-match"}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
...
func TestPurgeQueueHandler_PurgeInProgress(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("PurgeQueue", mock.Anything, "purging-queue").Return(store.ErrPurgeQueueInProgress)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueUrl": "http://localhost/queues/purging-queue"}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
...
func TestDeleteMessageBatchHandler_InvalidEntryId(t *testing.T) {
	mockStore := new(MockStore)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueUrl": "http://localhost/queues/my-queue", "Entries": [{"Id": ""}]}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
...
func TestChangeMessageVisibilityHandler_InvalidHandle(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("ChangeMessageVisibility", mock.Anything, "my-queue", "invalid-handle", 60).Return(store.ErrInvalidReceiptHandle)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueUrl": "http://localhost/queues/my-queue", "ReceiptHandle": "invalid-handle", "VisibilityTimeout": 60}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
	req.Header.Set("X-Amz-Target", "AmazonSQS.ChangeMessageVisibility")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestGetQueueAttributesHandler_InvalidAttribute(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("GetQueueAttributes", mock.Anything, "my-queue").Return(map[string]string{}, nil)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueUrl": "http://localhost/queues/my-queue", "AttributeNames": ["Invalid"]}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
	req.Header.Set("X-Amz-Target", "AmazonSQS.GetQueueAttributes")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestListQueuesHandler_InvalidToken(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("ListQueues", mock.Anything, 0, "invalid", "").Return(nil, "", nil)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"NextToken": "invalid"}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
	req.Header.Set("X-Amz-Target", "AmazonSQS.ListQueues")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestPurgeQueueHandler_QueueNotFound(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("PurgeQueue", mock.Anything, "non-existent").Return(store.ErrQueueDoesNotExist)
	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	body := `{"QueueUrl": "http://localhost/queues/non-existent"}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(body))
	req.Header.Set("X-Amz-Target", "AmazonSQS.PurgeQueue")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestCreateQueueHandler_FIFOAttributes(t *testing.T) {
	mockStore := new(MockStore)
	mockStore.On("CreateQueue", mock.Anything, "my-fifo-queue.fifo", mock.Anything, mock.Anything).Return(nil)

	app := &App{Store: mockStore}
	r := chi.NewRouter()
	app.RegisterSQSHandlers(r)

	// Test case for valid DeduplicationScope and FifoThroughputLimit
	validBody := `{"QueueName": "my-fifo-queue.fifo", "Attributes": {"FifoQueue": "true", "DeduplicationScope": "messageGroup", "FifoThroughputLimit": "perMessageGroupId"}}`
	req, _ := http.NewRequest("POST", "/", bytes.NewBufferString(validBody))
	req.Host = "localhost:8080"
	req.Header.Set("X-Amz-Target", "AmazonSQS.CreateQueue")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
	...
	// Test case for invalid combination
	invalidBody := `{"QueueName": "my-fifo-queue.fifo", "Attributes": {"FifoQueue": "true", "DeduplicationScope": "queue", "FifoThroughputLimit": "perMessageGroupId"}}`
	req, _ = http.NewRequest("POST", "/", bytes.NewBufferString(invalidBody))
	req.Host = "localhost:8080"
	req.Header.Set("X-Amz-Target", "AmazonSQS.CreateQueue")
	rr = httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestValidationFunctions(t *testing.T) {
	t.Run("validateIntAttribute", func(t *testing.T) {
		assert.NoError(t, validateIntAttribute("10", 0, 20))
		assert.Error(t, validateIntAttribute("21", 0, 20))
		assert.Error(t, validateIntAttribute("-1", 0, 20))
		assert.Error(t, validateIntAttribute("abc", 0, 20))
	})

	t.Run("validateAttributes", func(t *testing.T) {
		attrs := map[string]string{"DelaySeconds": "10"}
		assert.NoError(t, validateAttributes(attrs))

		attrs = map[string]string{"FifoQueue": "true"}
		assert.NoError(t, validateAttributes(attrs))

		attrs = map[string]string{"DelaySeconds": "abc"}
		assert.Error(t, validateAttributes(attrs))
	})

	t.Run("isValidMessageAttributeName", func(t *testing.T) {
		assert.True(t, isValidMessageAttributeName("abc"))
		assert.True(t, isValidMessageAttributeName("a.b-c_1"))
		assert.False(t, isValidMessageAttributeName("AWS.something"))
		assert.False(t, isValidMessageAttributeName(".abc"))
		assert.False(t, isValidMessageAttributeName("abc."))
		assert.False(t, isValidMessageAttributeName("a..b"))
	})
}

func TestUnimplementedHandlers(t *testing.T) {
	app := &App{}
	unimplementedHandlers := map[string]http.HandlerFunc{
		"SetQueueAttributes":          app.SetQueueAttributesHandler,
		"GetQueueUrl":                 app.GetQueueUrlHandler,
		"ChangeMessageVisibilityBatch": app.ChangeMessageVisibilityBatchHandler,
		"AddPermission":               app.AddPermissionHandler,
		"RemovePermission":            app.RemovePermissionHandler,
		"ListQueueTags":               app.ListQueueTagsHandler,
		"TagQueue":                    app.TagQueueHandler,
		"UntagQueue":                  app.UntagQueueHandler,
		"ListDeadLetterSourceQueues":  app.ListDeadLetterSourceQueuesHandler,
		"StartMessageMoveTask":        app.StartMessageMoveTaskHandler,
		"CancelMessageMoveTask":       app.CancelMessageMoveTaskHandler,
		"ListMessageMoveTasks":        app.ListMessageMoveTasksHandler,
	}

	for name, handler := range unimplementedHandlers {
